"""
gpu_shared.py — Same as gpu_global.py but uses shared-memory tiled flux kernel.
Block size must be (16, 16) for the fixed shared memory layout [17][17].
"""
...
def run_gpu_shared(nx, ny, target_times, CFL=DEFAULT_CFL,
                   precision='float32', block_size=(16, 16)):
    """Same interface as run_gpu_global but uses shared-memory tiled flux kernel."""
    import cupy as cp
    ...
    bx, by = (16, 16)   # shared kernel requires exactly 16×16
